...
| `typeset::font` | `FontSet` `FontDeclaration` | フォントをキー／family 名で引き、文字カバレッジでフォールバックを解決。同じ family の複数 face から weight / italic の最近傍を選ぶ（無ければフェイクボールド／斜体）。`declare()` はメタデータだけ登録して初回使用時に開く。`setLanguageFonts()` で言語ごとに先に試す family。シェイピング用の `hb_font_t` も持つ |
