...
| 層 | 主要な型 | 役割 |
|---|---|---|
| `typeset` | `Pt` `Point` `Rect` `Matrix` `Color` `Path`（geom.hpp）、`WritingMode`（writing_mode.hpp）、`TextStyle` `ParagraphStyle` `SpacingOptions` `BreakOptions`（style.hpp） | 幾何・書字方向・スタイル |
| `typeset::font` | `FontSet` | フォントをキー／family 名で引き、文字カバレッジでフォールバックを解決。シェイピング用の `hb_font_t` も持つ |
| `typeset::text` | `CharClass` `SpacingTable`（JLReq 附属書 A・表 3）、`orientation`（UAX #50）、`line_break`（UAX #14） | 文字クラスとアキ量、向き、分割機会 |
| `typeset::inl` | `Paragraph` `InlineRun` `Annotation`、`ParagraphLayouter` `ParagraphFragment` `LineBox`、`LineShapeProvider`、`shapeText()` | 行内組版。Box / Glue / Penalty 列を Greedy / Knuth–Plass で解く。ルビ・縦中横・圏点・割注・字取り・行内画像／オブジェクト・脚注記号 |
